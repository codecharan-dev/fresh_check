...
| 14 | [Git Workflow](#git-workflow)                     | Branches, commit conventions, push flow per environment       |
| 15 | [Key Packages](#key-packages)                    | Every dependency and why it's here                            |
| 16 | [Lint & Static Analysis](#lint--static-analysis) | Rules enforced, error promotions, suppression syntax          |
| 17 | [Naming Conventions](#naming-conventions)        | Files, classes, BLoC, models, entities, use cases             |

### Status
| #  | Section                                                  | What you'll find                                       |
|----|----------------------------------------------------------|--------------------------------------------------------|
| 18 | [Known Issues / In Progress](#known-issues--in-progress) | Splash, core layer, theme, bloc observer — all pending |
...
git clone https://github.com/codecharan-dev/fresh_check.git
cd fresh_check

# 2. Switch to the development branch
git checkout development

# 3. Install the pinned Flutter version
fvm install

# 4. Get dependencies
fvm flutter pub get

# 5. Run code generation (freezed / json_serializable)
fvm flutter pub run build_runner build --delete-conflicting-outputs

# 6. Run the app (dev flavor by default)
...
**Repository:** `https://github.com/codecharan-dev/fresh_check.git`

### Branch strategy

| Branch        | Environment | Purpose                    |
|---------------|-------------|----------------------------|
| `main`        | prod        | Production releases (tagged) |
| `staging`     | staging     | Pre-production validation  |
| `qa`          | qa          | QA / testing team          |
| `development` | dev         | Day-to-day development     |

All feature work happens on `development`. Merge flow: `development` → `qa` → `staging` → `main`.

### Commit message format

```
type(env): short description
```
...
Examples:
```bash
feat(dev): add scan camera screen
fix(qa): correct remote config base url
chore(staging): update firebase configuration
```
...
# ... make changes ...
git add <files>
git commit -m "feat(dev): add scan camera screen"
...
| Excluded | Reason |
|----------|--------|
| `ios/Pods/` | Regenerated via `pod install` from `Podfile.lock` (which IS committed) |
| `build/` | Build output |
| `.dart_tool/` | Dart tool cache |
| `android/.gradle/` | Gradle cache |
| `ios/Flutter/Generated.xcconfig` | Auto-generated by Flutter on every build |
| `*.env`, `*.keystore`, `*.jks` | Sensitive credentials |
| `key.properties` | Android signing config |
| `service-account*.json`, `firebase-adminsdk*.json` | Firebase admin keys |

### What IS committed (safe)

`google-services.json`, `GoogleService-Info.plist`, and `firebase_options_*.dart` contain non-secret Firebase client API keys. These are required for builds and [safe to commit per Firebase docs](https://firebase.google.com/docs/projects/api-keys).
